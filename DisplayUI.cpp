...
void DisplayUI::setup() {
    ...
    createMenu(&mainMenu, NULL, [this]() {
        ...
        addMenuNode(&mainMenu, D_PROBE_MONITOR, [this]() { // PROBE MONITOR
            scan.start(SCAN_MODE_SNIFFER, 0, SCAN_MODE_OFF, 0, false, wifi_channel);
            mode = DISPLAY_MODE::PROBEMONITOR;
        });
        addMenuNode(&mainMenu, "DEAUTH DETECTOR", [this]() { // PROBE MONITOR
            scan.start(SCAN_MODE_SNIFFER, 0, SCAN_MODE_OFF, 0, false, wifi_channel);
            mode = DISPLAY_MODE::DEAUTH;
        });
        addMenuNode(&mainMenu, D_EVIL_DETECTOR , [this]() {
           mode = DISPLAY_MODE::EVILMONITOR;
        });
        //addMenuNode(&mainMenu, [this]() {
			//return (settings::getWebSettings().enabled) ? D_AP_OFF : D_AP_ON; }, [this]() {
			//if (settings::getWebSettings().enabled) {
			//cli.runCommand("set webinterface \"false\""); cli.runCommand("save settings");
			//cli.runCommand("stopap"); } else { cli.runCommand("set webinterface \"true\"");
			//cli.runCommand("save settings"); cli.runCommand("startap"); }
		//});	// AP ON/OFF
    ...
    createMenu(&scanMenu, &mainMenu, [this]() {
        ...
        addMenuNode(&scanMenu, D_SCAN_ST_1, [this]() { // SCAN ST 60s
            scan.start(SCAN_MODE_STATIONS, 60000, SCAN_MODE_OFF, 0, true, wifi_channel);
            mode = DISPLAY_MODE::LOADSCAN;
        });
    });
    ...
    createMenu(&apListMenu, &showMenu, [this]() {
        ...
        for (int i = 0; i < c; i++) {
            addMenuNode(&apListMenu, [i]() {
                return b2a(accesspoints.getSelected(i)) + accesspoints.getSSID(i) + str(" | ") + accesspoints.getRSSI(i)  + str(" | ") + accesspoints.getCh(i);
            }, [this, i]() {
    ...
    createMenu(&stationListMenu, &showMenu, [this]() {
        ...
        for (int i = 0; i < c; i++) {
            addMenuNode(&stationListMenu, [i]() {
                ...
                (stations.hasName(i) ? stations.getNameStr(i) : stations.getMacVendorStr(i)) + str(" | ") + stations.getAPStr(i);
            }, [this, i]() {
    ...
    createMenu(&apMenu, &apListMenu, [this]() {
        ...
        addMenuNode(&apMenu, D_NAME, [this]() { // NAME
            names.add(accesspoints.getMacStr(selectedID), accesspoints.getSSID(selectedID),
            bssid, accesspoints.getCh(selectedID), true, false);
            changeMenu(&nameListMenu);
        });
    ...
    createMenu(&attackMenu, &mainMenu, [this]() {
        addMenuNode(&attackMenu, [this]() { // *DEAUTH 0/0
         ...
         addMenuNode(&attackMenu, [this]() { // *DEAUTH ALL 0/0
            if (attack.isRunning()) return leftRight(b2a(deauthSelected) + str(D_DEAUTH_ALL),
                                                     (String)attack.getDeauthPkts() + SLASH +
                                                     (String)attack.getDeauthMaxPkts(), maxLen - 1);
        ...
        addMenuNode(&attackMenu, [this]() { // EVIL TWIN
            return leftRight(b2a(evilSelected) + str(D_EVIL), " " + accesspoints.selected_target, maxLen - 1);
        }, [this]() { // evil twin
            evilSelected = !evilSelected;

            
        });
        addMenuNode(&attackMenu, [this]() { // ROGUE WIFI
            return leftRight(b2a(rogueSelected) + str(D_ROGUE), " " + ssids.rogue_wifi, maxLen - 1);
        }, [this]() { // evil twin
            rogueSelected = !rogueSelected;
        ...
        addMenuNode(&attackMenu, [this]() { // START
            ...
            if (attack.isRunning()) attack.stop();
              ...
              attack.start(beaconSelected, deauthSelected, deauthAllSelected, probeSelected, true,
                              settings::getAttackSettings().timeout * 1000);
             if(evilSelected == true){
                 attack.eviltwin = true;
                 wifi::eviltwinAP();
             }
             if(rogueSelected == true){
                wifi::rogueAP();
             }
        }
        });
...
void DisplayUI::update(bool force) {
	...
	if (mode != DISPLAY_MODE::PACKETMONITOR && mode != DISPLAY_MODE::PROBEMONITOR && mode != DISPLAY_MODE::DEAUTH && mode != DISPLAY_MODE::EVILMONITOR) {
...
void DisplayUI::setupButtons() {
    ...
    up->setOnClicked([this]() {
        ...
        if (!tempOff) {
            ...
            } else if (mode == DISPLAY_MODE::DEAUTH_ALL) {
				if (tempCount<=accesspoints.count()) {
				tempSSID = accesspoints.getSSID(tempCount);
				tempRSSI = accesspoints.getRSSI(tempCount);
				
				tempCount++;
				} else {
				tempSSID = accesspoints.getSSID(0);
				tempRSSI = accesspoints.getRSSI(0);
    ...
    down->setOnClicked([this]() {
        ...
        if (!tempOff) {
            if (mode == DISPLAY_MODE::MENU) {                 // when in menu, go up or down with cursor
				...
				isPaused = false;
				scan.start(SCAN_MODE_APS, 0, SCAN_MODE_APS, 30000, true, wifi_channel);
				}
			}
        }
    });
        ...
        if (!tempOff) {
            switch (mode) {
                ...
                case DISPLAY_MODE::PROBEMONITOR:
					...
					scan.probeSSID.clear();
					scan.probeMAC.clear();
					
					scan.stop();
                    mode = DISPLAY_MODE::MENU;
                    break;
                case DISPLAY_MODE::DEAUTH:
                    mode = DISPLAY_MODE::MENU;
                    break;    
                case DISPLAY_MODE::EVILMONITOR:
                    mode = DISPLAY_MODE::MENU;
                    break;  
...
void DisplayUI::drawProbeMonitor() {
    drawString(0, center(str("Pb:") + String(' ') + scan.probes + str("|") + str("Dth:") + String(' ') + scan.deauths, maxLen));
    if (scan.probeRSSI != 0) {
    drawString(1, String(scan.probeRSSI)); }
    drawString(2, center(scan.probeSSID, maxLen));
    drawString(4, center(scan.probeMAC, maxLen));
    Serial.print(String(scan.probeRSSI) + " " + scan.probeSSID +" " + scan.probeMAC);
}
void DisplayUI::drawDeauthMonitor() {
  drawString(0,String(scan.deauths) + " deauth/s | " + String(scan.probeRSSI));
...
String STATUSs;
unsigned long jeda_scan = 0;
int stat = 0;
bool sr = false;
int c;
void DisplayUI::drawEvilMonitor() {
  if(millis() - jeda_scan >= 10000){
    jeda_scan = millis();
    cli.runCommand("scan ap");
    stat = 0;
  }
  c = accesspoints.count();
  if(scan.isScanning()){
    sr = true;
  }
  if(!scan.isScanning() && sr == true){
    for(int i = 0;i < c;i++ ){
    if(accesspoints.selected_target == accesspoints.getSSID(i)){
      stat++;
    }
    prntln(accesspoints.getSSID(i));
   }
    prntln(stat);
    sr = false;
    if(stat >= 2){
  ...
  drawString(0,String("TARGET : " + accesspoints.selected_target));
  drawString(1,String("STATUS : " + STATUSs));
...
void DisplayUI::drawTrueDeauth() {
	...
	if(attack.isRunning()) {
		drawString(2, center(str(D_DEAUTH_ALL_1), maxLen)); }
		drawString(3, center(accesspoints.count() + String(' ')
		+ str(DSP_APS), maxLen));
	if (scan.isScanning()) {
		drawString(4, center(str(D_DEAUTH_ALL_2), maxLen)); }
	if (isPaused) {
		drawString(4, center(str(D_DEAUTH_ALL_3), maxLen)); }
}
...
//  drawImageCenterVertical(10, apple_width, apple_height, apple_bits);
   if (scan.isScanning()) {
     if(millis() - proses >= 10){
      maju++;
      proses = millis();
      drawString(maju,50,"=>>"+String(accesspoints.count()) +"/"+ String(stations.count()));
      display.display();
     }
